...
function MenuCard({ item }) {
  ...
  return (
    <div style={{ background:hov?'var(--cream)':'var(--bg)', padding:'20px 18px',
      borderBottom:hov?'2px solid var(--gold)':'2px solid transparent',
      cursor:'pointer', transition:'border-color 0.25s, background 0.2s' }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
    >
      <span style={{ fontSize:'30px', display:'block', marginBottom:'8px',
        transform:hov?'scale(1.12)':'scale(1)', transition:'transform 0.3s' }}>{item.emoji}</span>
      <div style={{ fontSize:'12px', fontWeight:500, color:'var(--text-1)', marginBottom:'3px' }}>{item.name}</div>
      <div style={{ fontSize:'9px', letterSpacing:'1px', textTransform:'uppercase', color:'var(--gold)', marginBottom:'4px' }}>{item.category}</div>
...
export default function MenuPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section style={{ padding:'64px 40px 40px', background:'linear-gradient(160deg,#0f0800 0%,#1a1208 50%,#2d1a00 100%)',
        textAlign:'center', borderBottom:'1px solid rgba(184,134,42,0.18)' }}>
      ...
      {/* Placeholder photos */}
      <section style={{ padding:'44px 20px', background:'var(--cream)', borderBottom:'1px solid var(--cream-3)' }}>
        <div style={{ maxWidth:'560px', margin:'0 auto', border:'2px dashed rgba(184,134,42,0.2)',
          padding:'44px 28px', textAlign:'center', background:'var(--bg)' }}>
          ...
          <p style={{ fontSize:'11px', color:'var(--text-2)', lineHeight:1.8, marginBottom:'20px' }}>
            Envoyez vos photos et elles seront intégrées directement.<br/>
            La structure est déjà prête.
          </p>
          <button className="btn btn-outline-gold">Envoyer les photos</button>
        </div>
      </section>

      {/* Filtres + grille */}
      <section style={{ padding:'44px 20px 72px', background:'var(--bg)' }}>
          ...
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'28px', overflowX:'auto', paddingBottom:'4px' }}>
            {MENU_CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActive(cat)} className="btn"
                style={{ background:active===cat?'var(--gold)':'transparent',
                  border:active===cat?'1px solid var(--gold)':'var(--border)',
                  color:active===cat?'#fff':'var(--text-2)', padding:'7px 18px', whiteSpace:'nowrap' }}>
                {cat}
              </button>
            ))}
          </div>
