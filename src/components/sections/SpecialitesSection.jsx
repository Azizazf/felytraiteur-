...
function Card({ item, onClick }) {
  ...
  return (
    ...
    >
      {/* Image */}
      <div style={{ height:'200px', position:'relative', overflow:'hidden', background:'var(--cream)' }}>
        {item.image && !err ? (
        ...
        <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,.2)', opacity:hov?1:0, transition:'opacity .3s' }}/>
        <div style={{
          position:'absolute', bottom:0, left:0, right:0, zIndex:3,
          background: hov ? 'var(--gold)' : 'rgba(14,10,4,.8)',
          padding:'9px 14px', fontSize:'10px', letterSpacing:'2px',
          textTransform:'uppercase', color:hov?'#111':'#fff',
          fontFamily:'var(--font-b)', transition:'all .3s',
        }}>{item.label}</div>
      </div>

      {/* Body */}
      <div style={{ padding:'20px 18px', borderTop:hov?'2px solid var(--gold)':'2px solid transparent', transition:'border-color .3s' }}>
        <h3 style={{ fontSize:'14px', fontWeight:500, color:'var(--text-1)', marginBottom:'8px' }}>{item.title}</h3>
        <p style={{ fontSize:'12px', color:'var(--text-2)', lineHeight:1.7,
          display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden',
        }}>{item.desc}</p>
        {/* Indicateur "cliquer pour voir plus" */}
        <div style={{
          display:'flex', alignItems:'center', gap:'6px',
          fontSize:'10px', color:'var(--gold)', letterSpacing:'1px', marginTop:'14px',
          opacity: hov ? 1 : 0.4,
